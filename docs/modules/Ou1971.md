...
\end{align}
```
where ``\sigma`` is the eigenvalue, ``\mathsf{X}=[\widetilde{w}, \widetilde{\zeta}, \widetilde{b}]^T`` is the eigenvector and the matrices ``\mathsfit{A}``, ``\mathsfit{B}`` are the complex and real non-symmetric matrices, respectively. 
